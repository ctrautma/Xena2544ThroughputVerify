...
class XenaSocketDriver(SimpleSocket):
    ...
    def __init__(self, hostname, port=22611):
        ...
        """
        SimpleSocket.__init__(self, hostname=hostname, port=port, timeout=5)
...
class XenaStream(object):
    ...
    def set_packet_per_second(self, pps):
        """Set the packet per second

        :param pps: number of packets per second
        :return: Boolean True if success, False if error
        """
        return self._manager.driver.ask_verify(make_stream_command(
            CMD_SET_STREAM_PPS, pps, self))
